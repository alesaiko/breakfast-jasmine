...
/*
 * Use FB notifier to automatically enable file synchronization after LCD
 ...
 * states quite often. If FB notifier is unused in the kernel, this will
 * be dropped.
 */
#define DEF_FB_NOTIFIER_IS_USED	(1)
 ...
 * usage of FB notifier and a timer fire after dyn_sync.delay milliseconds.
				...
				unsigned long event, void *data)
{
	...
	if (likely(!dyn_sync.enabled || !dyn_sync.fb_notify_used))
		return NOTIFY_OK;
					  ...
					  const char *buf, size_t count)
{
	 ...
	 * If FB notifier is not used, just change the synchronization
	 * state according to an input value.
	 */
	if (dyn_sync.enabled && dyn_sync.fb_notify_used &&
	    dyn_sync.screen_state >= 0)
...
static ssize_t show_fb_notify_used(struct kobject *kobj,
				   struct kobj_attribute *attr,
				   char *buf)
{
	...
	return scnprintf(buf, 3, "%u\n", dyn_sync.fb_notify_used);
}

static ssize_t store_fb_notify_used(struct kobject *kobj,
				    struct kobj_attribute *attr,
				    const char *buf, size_t count)
{
	int ret;
	...
	ret = kstrtouint(buf, 2, &val);
	if (ret || val == dyn_sync.fb_notify_used)
		return -EINVAL;

	dyn_sync.fb_notify_used = val;
	 ...
	 * available instantly after FB notify enablement without the
	 * requirement to turn the screen on/off. In case user wants to throw
	 * FB notify support away, just disable fsync as he might expect this
	 * to happen.
	 */
	 ...
	 * if FB notify is going to be dropped.
	 */
...
static struct kobj_attribute dynamic_sync_fb_notify =
	__ATTR(Dyn_fsync_fb_notify, 0644,
		show_fb_notify_used, store_fb_notify_used);

static ssize_t show_dynamic_sync_delay(struct kobject *kobj,
				       struct kobj_attribute *attr,
				       char *buf)
{
	if (!dyn_sync.fb_notify_used)
		return scnprintf(buf, 15, "<unsupported>\n");
...
static int __init dynamic_sync_init(void)
{
	...
	/*
	 * Do not fail if FB notifier registration failed as it is
	 * expected to fail in case it is not used in kernel tree.
	 */
	ret = fb_register_client(&fb_notifier_block);
	if (ret < 0) {
		pr_err("Unable to register FB notifier\n");
		dyn_sync.fb_notify_used = dyn_sync.delay = 0;
	}
